    ...
    const patientsUnsub = onSnapshot(
      ...
      (snap) => {
        ...
        const entries = snap.docs.map(d => ({ 
          ...
          message: `Medical record ${d.id} initialized.`,
    ...
    <div className="space-y-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 font-medium">A real-time snapshot of MedFlow AI infrastructure.</p>
        </div>
            ...
            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, i) => (
              <div key={day} className="flex flex-col items-center gap-4 w-12 h-full justify-end group">
                <div className="flex gap-1 items-end w-full h-full">
                  <div className="w-1/2 bg-[#2C332F] rounded-t-md" style={{ height: `${[45, 65, 40, 80, 50, 45, 10][i]}%` }}></div>
                  <div className="w-1/2 bg-[#3D7C65] rounded-t-md" style={{ height: `${[60, 40, 75, 55, 65, 30, 5][i]}%` }}></div>
                </div>
          ...
          <div className="space-y-6 flex-1">
            {logs.length === 0 ? (
              <div className="bg-white rounded-[32px] p-10 flex flex-col items-center text-center justify-center opacity-40">
                ...
                <p className="text-xs font-bold uppercase tracking-widest">Syncing realtime data...</p>
              </div>
            ) : logs.map((log) => {
              const isNew = log.id === newLogId;
              const isLab = log.type === 'LAB';
              const isDelivery = log.type === 'DELIVERY';
              const lineColor = isLab ? 'bg-[#3D7C65]' : isDelivery ? 'bg-blue-400' : 'bg-amber-400';
                ...
                <div key={log.id} className={`flex gap-6 items-start transition-all duration-700 ${isNew ? 'scale-105 origin-left' : ''}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 font-medium mt-1 leading-relaxed">
                      {isLab 
                        ? `${log.patientName || 'Patient'}'s report successfully uploaded.`
